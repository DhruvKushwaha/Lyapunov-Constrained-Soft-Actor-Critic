"""
Grid-search EDMD + LQR hyperparameters for 2D quadrotor tracking-error dynamics.
...
Note: q_phi directly controls P conditioning — higher q_phi penalises RBF features more,
pulling P_min_eig up and reducing the condition number at the cost of diluting the purely
tracking-error interpretation of V(z).

Usage
-----
  # Quick scan (coarse grid, ~54 combos):
  python -m edmd.tune_quad_2d --quick

  # Full scan (~300 combos):
  python -m edmd.tune_quad_2d

  # Re-train the winner and save model + matrices:
  python -m edmd.tune_quad_2d --quick --retrain-best

  # Custom data file / output dir:
  python -m edmd.tune_quad_2d --data results/edmd/data/quadrotor_2d_track.npz --output results/edmd/quadrotor_2d_track
"""
...
QUICK_GRID = {
    "n_rbf_centers": [3, 8, 16],
    "rbf_width":     [0.1, 0.5, 1.0],
...
# ---------------------------------------------------------------------------
# EDMD training (reuses logic from train_quad_2d.py)
# ---------------------------------------------------------------------------
def _train_edmd(X, X_next, U, n_rbf_centers, rbf_width, regularization):
    ...
    kmeans.fit(X)
    centers = kmeans.cluster_centers_.T  # (n_state, n_centers)
...
# ---------------------------------------------------------------------------
# LQR / DARE sweep (fast — no EDMD retraining)
# ---------------------------------------------------------------------------
def _try_lqr(A, B, n_error, q_x, q_phi):
    """
    Attempt DARE solve with given cost weights.  Returns (metrics_dict, passed)
    where `passed` is True only when all control-theory criteria are met.
    """
    ...
    # Hard criteria: physical requirements (not a tuning threshold)
    passed = (
...
def run_search(X, X_next, U, grid, verbose):
    ...
    valid = []      # passed hard criteria (P>0, A_cl<1, controllable)
    invalid = []    # failed hard criteria
    edmd_failed = 0
...
def main() -> int:
    ...
    parser.add_argument("--p-cond-max", type=float, default=1e9,
                        help="P condition number threshold for the ✓ marker in the table "
                             ...
                             "this only affects the ok column annotation.")
    parser.add_argument("--data", type=str, default="results/edmd/data/quadrotor_2d_track.npz",
                        help="Path to .npz data file (default: results/edmd/data/quadrotor_2d_track.npz)")
    parser.add_argument("--output", type=str, default="results/edmd/quadrotor_2d_track",
    ...
    if not valid:
        print(
            ...
            "Suggestions:\n"
            "  • Try larger n_rbf_centers (more expressive lifting)\n"
            "  • Try smaller rbf_width (sharper RBF locality)\n"
            "  • Check data quality: run edmd.collect_quad_2d with more episodes"
        )
        return 1
    ...
    log_cond = math.log10(p_cond_best) if math.isfinite(p_cond_best) and p_cond_best > 0 else float("nan")
    n_meet_threshold = sum(1 for e in valid if e.get("P_condition_number", float("inf")) <= args.p_cond_max)
    ...
    print(f"\n  {n_meet_threshold}/{len(valid)} valid configs meet p_cond_max={args.p_cond_max:.0e}")
    ...
    if args.retrain_best:
        ...
        print(
            ...
            f"  python -m edmd.tune_quad_2d {'--quick ' if args.quick else ''}--retrain-best"
        )
