#!/usr/bin/env python3
"""
Collect 3D quadrotor PID rollouts for EDMDc (uses ``PID_controller_quadrotor.collect_edmd_data`` with 3D task YAML).

Output defaults to ``results/edmd/data/quadrotor_3d.npz``; includes ``episode_id`` like the 2D collector.

Examples::

  python -m edmd.collect_quad_3d
  python -m edmd.collect_quad_3d --trajectories figure8,circle --num-episodes 25
"""
...
def main() -> int:
    ...
    p.add_argument("--env-yaml", type=str, default="Params/Quadrotor_3D/pid/env_track.yaml")
    ...
    p.add_argument("--output", type=str, default="results/edmd/data/quadrotor_3d.npz")
    p.add_argument(
        "--also-offline-npz",
        type=str,
        default=None,
        help=(
            ...
            "Requires finite RL rewards (e.g. Params/Quadrotor_3D/env_track_rl.yaml)."
        ),
    )
    ...
    qt = env_config.get("quad_type")
    if qt != 3:
        print(
            f"Warning: task_config quad_type is {qt!r}, expected 3 for 3D EDMD.",
            file=sys.stderr,
        )

    pid_cfg = load_pid_config(pid_yaml)

    print("Collecting EDMD discrete data (3D PID baseline)...")
    ...
    meta = {
        ...
        "normalized_action_space": args.normalized_actions,
        "output": str(out_path),
        "train_module": "edmd.train_quad_3d",
        "shapes": {
    ...
    if also_off:
        ...
        try:
            ...
            print(
                "Hint: use a 3D env YAML with `cost: rl_reward` (e.g. Params/Quadrotor_3D/env_track_rl.yaml) "
                "for `--also-offline-npz`.",
                file=sys.stderr,
            )
            return 3
